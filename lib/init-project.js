...
let runProject = (platform = 'ios') => {
  return new Promise((resolve, reject) => {
    ...
    let data = '';
    try {
      data = fs.readFileSync(config).toString();
    } catch (e) {
      reject('NOT an RNRails project!');
    }
...
let configureJest = () => {
  return new Promise((resolve, reject) => {
    ...
    configuration.jest['setupFiles'] = ["./jest-setup"];
...
let installDevDependencies = () => {
  return new Promise((resolve, reject) => {
    const cmd = 'yarn add -D chai enzyme enzyme-adapter-react-16 sinon';
    const cwd = process.cwd();
    const options = {
      encoding: 'utf8',
      timeout: 0,
