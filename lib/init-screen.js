...
let initScreen = (name, options) => {
  return new Promise((resolve, reject) => {
    ...
    let content = definitions.component.replace('MyScreen', componentName).replace('MyScreen', componentName).replace('Escaped', escapedName).replace('Escaped', escapedName);
    fs.writeFile(file, content, err => {
      if (err) reject(err);
      fs.writeFileSync('./src/app.js', insertScreenDefinition(componentName, fileName));
      resolve();
    });
  });
}
